...
namespace wallysworld
{
	...
	inline int wallyRun(TConfigStruct &c)
	{
		...
		std::stringstream check1(c.regionLabel);
		std::string raw;
		// Split the string by ','
		while (getline(check1, raw, ','))
		{
			labels.push_back(raw);
		}

		// Check if the number of labels matches the number of splits 2024-6-13 jc ： not to check
		// if (labels.size() <= c.splits)
		// {
		// 	std::cerr << "The number of labels less than the number of splits." << std::endl;
		// 	return 1;
		...
		for (uint32_t rgIdx = 0; rgIdx < rg.size(); ++rgIdx)
		{
			...
			if (!parseAnnotation(hdr, c, rg[rgIdx], tr, anno))
				return 1;
			...
			for (int32_t i = 0; i < headerTracks; ++i)
				taken[i] = WALLY_BLOCKED;
			...
			for (unsigned int file_c = 0; file_c < c.files.size(); ++file_c)
			{
				...
				for (int32_t i = headerTracks; i < maxTracks; ++i)
				{
					if (i < lowerBound)
						...
						taken[i] = WALLY_UNBLOCK;
				}
				// Reserve coverage track
				for (int32_t i = lowerBound; i < lowerBound + 2; ++i)
					taken[i] = WALLY_BLOCKED;
				...
				{
					if (rec->core.flag & (BAM_FQCFAIL | BAM_FDUP | BAM_FUNMAP | BAM_FSECONDARY))
						continue;
					if ((rec->core.qual < c.minMapQual) || (rec->core.tid < 0))
						continue;
					if ((!c.showSupplementary) && (rec->core.flag & BAM_FSUPPLEMENTARY))
						continue;
					...
					if (c.showPairs)
					{
						...
						if (pl == 0)
						{
							if (std::abs((int)rec->core.pos - (int)rec->core.mpos) >= minSep)
								readCol = cv::Scalar(175, 175, 63);
						}
						else if (pl == 1)
						{
							if (std::abs((int)rec->core.pos - (int)rec->core.mpos) >= minSep)
								readCol = cv::Scalar(213, 100, 78);
						}
						else if (pl == 2)
						{
							if (std::abs(rec->core.isize) > sampleLib[file_c].median + c.madCutoff * sampleLib[file_c].mad)
								readCol = cv::Scalar(63, 63, 213);
						}
						else if (pl == 3)
						{
							if (std::abs((int)rec->core.pos - (int)rec->core.mpos) >= minSep)
								readCol = cv::Scalar(63, 175, 63);
						}
					...
					for (std::size_t i = 0; i < rec->core.n_cigar; ++i)
					{
						if ((bam_cigar_op(cigar[i]) == BAM_CMATCH) || (bam_cigar_op(cigar[i]) == BAM_CEQUAL) || (bam_cigar_op(cigar[i]) == BAM_CDIFF))
						{
							...
							if ((rp + bam_cigar_oplen(cigar[i]) == (uint32_t)alnend) && (!(rec->core.flag & BAM_FREVERSE)))
								drawTriangle = true;
							...
							for (std::size_t k = 0; k < bam_cigar_oplen(cigar[i]); ++k)
							{
								...
								if ((rpadj >= 0) && (rpadj < (int32_t)rg[rgIdx].size))
								{
									if (sequence[sp] == 'A')
									{
										if (covA[rpadj] < maxCoverage)
											++covA[rpadj];
									}
									else if (sequence[sp] == 'C')
									{
										if (covC[rpadj] < maxCoverage)
											++covC[rpadj];
									}
									else if (sequence[sp] == 'G')
									{
										if (covG[rpadj] < maxCoverage)
											++covG[rpadj];
									}
									else if (sequence[sp] == 'T')
									{
										if (covT[rpadj] < maxCoverage)
											++covT[rpadj];
									}
								}
						...
						{
							if (sp == 0)
								leadingSC = bam_cigar_oplen(cigar[i]);
							else
								drawSC(c, rg[rgIdx], bg, trackIdx, (rp - rg[rgIdx].beg), bam_cigar_oplen(cigar[i]), false);
							sp += bam_cigar_oplen(cigar[i]);
						}
				...
				drawSampleLabel(c, lowerBound + 3, labels[rgIdx], bg);
			}
			drawBorder(c, bg);
	...
	int region(int argc, char **argv)
	{
		...
		generic.add_options()("help,?", "show help message")("genome,g", boost::program_options::value<boost::filesystem::path>(&c.genome), "genome fasta file")("bed,b", boost::program_options::value<boost::filesystem::path>(&c.bedFile), "BED annotation file (optional)");

		boost::program_options::options_description disc("Graphics options");
		disc.add_options()("map-qual,q", boost::program_options::value<uint32_t>(&c.minMapQual)->default_value(1), "min. mapping quality")("mad-cutoff,m", boost::program_options::value<uint16_t>(&c.madCutoff)->default_value(9), "insert size cutoff, median+m*MAD")("snv-vaf,v", boost::program_options::value<float>(&c.snvvaf)->default_value(0.2), "min. SNV VAF")("snv-cov,t", boost::program_options::value<uint32_t>(&c.snvcov)->default_value(10), "min. SNV coverage")("region,r", boost::program_options::value<std::string>(&c.regionStr)->default_value("chr16:35-78,chr11:22-56"), "region to display, use ',' for multi-region")("label,l", boost::program_options::value<std::string>(&c.regionLabel)->default_value("HBA,HBB"), "region Labels to display, use ',' for multi-region")("rfile,R", boost::program_options::value<boost::filesystem::path>(&c.regionFile), "BED file with regions to display")("paired,p", "paired-end view")("supplementary,u", "show supplementary alignments")("clip,c", "show soft- and hard-clips");

		boost::program_options::options_description geno("Display options");
		geno.add_options()("split,s", boost::program_options::value<uint16_t>(&c.splits)->default_value(1), "number of horizontal images, for merge multi-region in one")("width,x", boost::program_options::value<uint32_t>(&c.width)->default_value(1024), "width of the plot")("height,y", boost::program_options::value<uint32_t>(&c.height)->default_value(1024), "height of the plot");

		// Define hidden options
		boost::program_options::options_description hidden("Hidden options");
		hidden.add_options()("input-file", boost::program_options::value<std::vector<boost::filesystem::path>>(&c.files), "input file")("window,w", "show window");
		...
		if (vm.count("clip"))
			...
			c.showSoftClip = false;

		// Supplementary
		if (vm.count("supplementary"))
			c.showSupplementary = true;
		else
			c.showSupplementary = false;
